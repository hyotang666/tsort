  ...
  (loop :for node1 :in rest
	...
	:if(find subject1 rest
	     :test(lambda(subject1 node)
		    (destructuring-bind(subject2 . direction2)node
		      (and (find subject1 direction2 :test test :key key)
			   (find subject2 direction1 :test test :key key)))))
	:collect it :into node2 :and
	:do(return(cons node1 node2))))
